...
const getTasks = (req: IncomingMessage, res: ServerResponse) => {
 ...
 const postTasks= (req: IncomingMessage, res: ServerResponse) => {
    ...
    req.on("end", () => {
      ...
      Fs.readFile(Path.join(__dirname, "store.json"), "utf8", (err, data) => {
        ...
        if (err) {
          ...
          let tasks: [Tasks] = JSON.parse(data);
          // get the id of the latest task
          let latest_id = tasks.reduce(
            (max = 0, task: Tasks) => (task.id > max ? task.id : max),
            0
          );
 ...
 const putTasks= (req: IncomingMessage, res: ServerResponse) => {
    // Read the data from the request
    let data = "";
    req.on("data", (chunk) => {
      data += chunk.toString();
    ...
    req.on("end", () => {
      // Parse the data
      let task: Tasks = JSON.parse(data);
      // Read the store.json file
      Fs.readFile(Path.join(__dirname, "store.json"), "utf8", (err, data) => {
        // Check out any errors
        if (err) {
 ...
 const deleteTasks= (req: IncomingMessage, res: ServerResponse) => {
    // Read the data from the request
    let data = "";
    req.on("data", (chunk) => {
      data += chunk.toString();
